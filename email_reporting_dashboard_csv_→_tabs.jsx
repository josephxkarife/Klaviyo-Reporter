...
function guessColumnMapping(headers) {
  ...
  const idx = (patterns) => {
    for (let i = 0; i < H.length; i++) {
      const h = H[i];
      if (patterns.some((p) => (p instanceof RegExp ? p.test(h) : h.includes(p))))
        return i;
    }
    return -1;
  };
...
function buildCanonicalRows(headers, rows) {
  ...
  for (const r of rows) {
    ...
    const saleGroup = type === "sale" ? saleGroupNameFromCampaign(campaignName) : "";
...
function dedupeByEmail(rows) {
  ...
  for (const r of rows) {
    ...
    // Recompute rates
    merged.openRate = merged.recipients > 0 ? clamp01(merged.opens / merged.recipients) : 0;
    merged.clickRate = merged.recipients > 0 ? clamp01(merged.clicks / merged.recipients) : 0;

    // Audience: if mixed, set unknown
    merged.audience =
      prev.audience === r.audience ? prev.audience : "unknown";
          ...
          {rows.map((r, idx) => (
            <tr key={idx} className={idx % 2 ? "bg-white" : "bg-gray-50/40"}>
              ...
              <td className="px-3 py-2 border-b align-top">
                <div className="font-medium text-gray-900">{r.campaignName}</div>
              </td>
              <td className="px-3 py-2 border-b align-top whitespace-nowrap">
                {r.sendDate.toLocaleString()}
              </td>
              <td className="px-3 py-2 border-b align-top">{formatPct(r.openRate)}</td>
              <td className="px-3 py-2 border-b align-top">{formatPct(r.clickRate)}</td>
              <td className="px-3 py-2 border-b align-top whitespace-nowrap">
                {formatMoney(r.revenue)}
              </td>
              <td className="px-3 py-2 border-b align-top">{r.uniqueOrders || ""}</td>
              <td className="px-3 py-2 border-b align-top">{r.recipients || ""}</td>
              <td className="px-3 py-2 border-b align-top">{r.monthSent}</td>
              <td className="px-3 py-2 border-b align-top max-w-[360px] truncate" title={r.subject}>
                {r.subject}
              </td>
              <td className="px-3 py-2 border-b align-top max-w-[360px] truncate" title={r.preview}>
                {r.preview}
              </td>
            </tr>
          ))}
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-sm text-gray-700">
            Avg Open: <span className="font-semibold">{formatPct(summary.avgOpenRate)}</span> · Avg Click:{" "}
            <span className="font-semibold">{formatPct(summary.avgClickRate)}</span>
          </div>
        </div>
        <div className="text-lg font-semibold text-gray-900">{formatMoney(summary.revenue)}</div>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const globalKPIs = useMemo(() => {
    ...
    const emailsCount = dedupeMode === "email" ? all.length : dedupeByEmail(all).length;
  ...
  const tabData = useMemo(() => {
    ...
    const saleNoHype = all.filter((r) => r.type === "sale");
    const saleGroups = groupBy(saleNoHype, (r) => r.saleGroup || "(Unlabeled Sale)");
  ...
  const tabs = useMemo(
    () => [
      ...
      { key: "buyers_evergreen_monthly", label: "Buyers — Evergreen (Monthly)" },
    ...
    return (
      <div className="rounded-2xl border bg-white p-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-semibold text-gray-900">Auto-mapped columns</div>
            <div className="text-xs text-gray-500">We’ll use these as the single source of truth.</div>
          </div>
          <div className="text-xs text-gray-500">Rows loaded: {parsed.rows.length}</div>
        </div>
  ...
  const Controls = () => (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="text-xs text-gray-500">
        30-day window: Revenue {formatMoney(rolling30.revenue)} · Avg Open {formatPct(rolling30.avgOpenRate)} · Avg Click {formatPct(rolling30.avgClickRate)}
      </div>
    </div>
  );
        ...
        {months.map((m) => {
          ...
          return (
            <div key={m} className="space-y-3">
              ...
              <Table rows={rs} />
              {includeSummary ? <SummaryRow label={`${m} summary`} summary={summary} /> : null}
            </div>
          );
        })}
      </div>
          ...
          return (
            <div key={name} className="space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <div className="text-xs text-gray-500">{rs.length} emails in sale</div>
              </div>
              <Table rows={rs} />
              <SummaryRow label="Sale summary (display-only)" summary={summary} />
            </div>
          );
        })}
        {!names.length ? (
          <div className="rounded-2xl border bg-white p-6 text-sm text-gray-600">
            No sale emails found ("sale", "special offer", or "launch" in campaign name). Note: "hype" emails are excluded from this tab’s calculations.
          </div>
        ) : null}
      </div>
    );
  ...
  const Charts = () => {
    ...
    return (
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          <div className="text-sm font-semibold text-gray-900">Revenue by month</div>
        ...
        <div className="rounded-2xl border bg-white p-4 shadow-sm">
          <div className="text-sm font-semibold text-gray-900">Evergreen vs sale revenue</div>
          <div className="mt-3 h-64">
            <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="mt-2 text-xs text-gray-500">Sale line excludes hype by definition of sale classification (hype is separate type).</div>
        </div>

        <div className="rounded-2xl border bg-white p-4 shadow-sm lg:col-span-2">
          <div className="text-sm font-semibold text-gray-900">Open + click rate trends</div>
          <div className="mt-3 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={monthRollup}>
                <CartesianGrid strokeDasharray="3 3" />
  ...
  const ActiveTabContent = () => {
    ...
    switch (activeTab) {
      ...
      case "evergreen_monthly":
        return <MonthlyBlocks map={tabData.evergreenByMonth} includeSummary={false} />;
      case "sale_monthly":
        return <MonthlyBlocks map={tabData.saleByMonth} includeSummary={false} />;
      case "buyers_evergreen_monthly":
        return <MonthlyBlocks map={tabData.buyersEvergreenMonthly} includeSummary={true} />;
      case "leads_evergreen_monthly":
        return <MonthlyBlocks map={tabData.leadsEvergreenMonthly} includeSummary={true} />;
      case "sale_grouped":
        return <SaleGrouped />;
      case "totals_monthly":
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-7xl p-5 sm:p-8">
        <div className="flex flex-col gap-4">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <div className="text-lg font-semibold text-gray-900">Email Performance Dashboard</div>
              <div className="text-sm text-gray-600">One raw CSV in. Executive-readable reporting out.</div>
            </div>
            <Controls />
          </div>

          <div className="grid grid-cols-1 gap-3 md:grid-cols-3 lg:grid-cols-6">
            <Card title="Total revenue" value={formatMoney(globalKPIs.totalRevenue)} />
            <Card title="Evergreen revenue" value={formatMoney(globalKPIs.evergreenRevenue)} />
            <Card title="Sale revenue" value={formatMoney(globalKPIs.saleRevenue)} sub="Excludes hype" />
            <Card title="Avg revenue / email" value={formatMoney(globalKPIs.avgRevPerEmail)} sub={dedupeMode === "email" ? "Email view" : "By email (deduped)"} />
          ...
          <div className="text-xs text-gray-500">
            Classification rules: sale if name contains "sale", "special offer", or "launch". Hype if name contains "hype" (excluded from sale calculations).
          </div>
        </div>
      </div>
    </div>
